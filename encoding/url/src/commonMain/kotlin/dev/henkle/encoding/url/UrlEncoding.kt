/*
Copyright 2025 Geert Bevin and Erik C. Thauvin
...
package dev.henkle.encoding.url

import dev.henkle.datastructures.bitset.BitSet
...
object UrlEncoding {
    ...
    private val unreservedChars = BitSet(size = 'z'.code + 1).apply {
        set('-'.code)
        set('.'.code)
    ...
    fun decode(
        ...
        while (i < length) {
            ...
            if (ch == '%') {
                ...
                try {
                    val v = src.substring(startIndex = i, endIndex = i + 2).toInt(radix = 16)
     ...
     * encoding.
     *
     * - Letters, numbers, unreserved (`-._~`) characters are left intact.
     */
    fun encode(
        src: String,
        allow: String = "",
        ...
        val allowedChars = allow.map { it }.toHashSet()
        if (src.isEmpty()) {
            return src
        }
        ...
        while (i < src.length) {
            ch = src[i]
            if (ch.isUnreserved() || ch in allowedChars) {
                out?.append(ch)
                i++
            } else {
                if (out == null) {
                ...
                when {
                    // ascii is encoded the same in both utf8 and utf16
                    cp < 0x80 -> {
                        if (spaceToPlus && ch == ' ') {
                            out.append('+')
                        } else {
                    ...
                    // supplementary planes are encoded with 2 bytes. Both bytes will be in the range 0xD800..0xDFFF
                    Char.isSupplementaryCodePoint(cp) -> {
                        val highSurrogate = ch
                        val lowSurrogate = src[i + 1]
                        charArrayOf(highSurrogate, lowSurrogate).concatToString()
                            .encodeToByteArray()
                            .map { it.toUByte().toString(radix = 16).uppercase() }
                            .forEach { byte ->
                                out.append('%')
                                out.append(byte)
                            }
                    ...
                    // BMP plane is encoded with 1 byte
                    else -> {
    ...
    private fun Char.Companion.isSupplementaryCodePoint(codePoint: Int): Boolean =
        codePoint in 0xD800..0xDFFF

    private fun String.codePointAt(index: Int): Int = this[index].code
}
